...
def load_dataset(
    ...
    """
    image_dir_path = os.path.join(folder_path, images_dir)
    scribble_dir_path = os.path.join(folder_path, scribbles_dir)
    
    # Get basenames from the image directory (without extension)
    image_filenames = _get_file_names(image_dir_path)
    basenames = [os.path.splitext(fname)[0] for fname in image_filenames]

    # Construct file paths based on basenames
    image_filepaths = [os.path.join(image_dir_path, f"{name}.jpg") for name in basenames] # Assumes .jpg
    scribble_filepaths = [os.path.join(scribble_dir_path, f"{name}.png") for name in basenames] # Assumes .png

    # Load the images and scribbles
    images = np.stack([_open_image(file, "RGB") for file in image_filepaths])
    scribbles = np.stack([_open_image(file, "grayscale") for file in scribble_filepaths])
    
    # Filenames for saving predictions should match the input
    pred_filenames = [f"{name}.png" for name in basenames]

    if ground_truth_dir is None:
        return images, scribbles, pred_filenames
    
    gt_dir_path = os.path.join(folder_path, ground_truth_dir)
    gt_filepaths = [os.path.join(gt_dir_path, f"{name}.png") for name in basenames] # Assumes .png
    
    ground_truth = np.stack([_open_image(file, None) for file in gt_filepaths])
    palette = Image.open(gt_filepaths[0]).getpalette()

    return images, scribbles, ground_truth, pred_filenames, palette
...
def visualize(
    ...
    alpha: float=0.6,
    pred_title: str = "Model Prediction"
):
    """
    ...
    Parameters:
        ...
        pred_title (str)       : title for the prediction plot
    """
